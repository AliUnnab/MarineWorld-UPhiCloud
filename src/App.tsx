...
export default function App() {
  ...
  // CANONICAL PERSONAL WORKSPACE ROUTES:
  if (
    path === "/workspace" ||
    ...
    if (path === "/saved/companies") initialTab = "companies";
  ...
  // 1. /explore, /cities, /sectors, /industries (or /categories) -> Maritime Categories / Industry Domains Taxonomy
  if (
    path === "/explore" ||
    path === "/cities" ||
    path === "/cities/" ||
  ...
  // 1.5. /companies -> Verified Maritime Companies Directory
  if (path === "/companies" || path === "/companies/" || path === "/network") {
  ...
  // 4. /companies/:companySlug / :moduleSlug / :productSlug | :serviceSlug -> Company Digital Operating Environment
  if (path.startsWith("/companies/")) {
    const parts = path.split("/").filter(Boolean);
    const companySlug = parts[1] || "crest-group-materials";
    const moduleSlug = parts[2] || "solutions";
    const productSlug = parts[2] === "products" ? parts[3] : undefined;
    const serviceSlug = parts[2] === "services" ? parts[3] : undefined;
    return (
      <CompanyPage
        config={config}
