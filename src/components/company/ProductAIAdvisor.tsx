...
import type { ProductEntity, CompanyProfile, ServiceEntity } from "@/lib/types";
import { executeProductAIQuery } from "@/lib/services/aiDomainService";
...
interface ProductAIAdvisorProps {
  product: ProductEntity;
  company: CompanyProfile;
  relatedServices?: ServiceEntity[];
  onInquire?: (product: ProductEntity) => void;
}

export function ProductAIAdvisor({ product, company, relatedServices, onInquire }: ProductAIAdvisorProps) {
  ...
  const defaultPrompts = [
    ...
    "What information is available about installation?",
  ...
  const handleAsk = async (questionText: string) => {
    ...
    try {
      const res = await executeProductAIQuery(
        company,
        product,
        authUid,
        questionText,
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-[11px] text-stone block font-medium">
              Technical & commercial advisory for {product.name}
            </span>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          >
            <input
              ...
              placeholder={`Ask custom question about ${product.name}...`}
              disabled={loading}
            ...
            <div className="rounded-card-md border border-line bg-white p-5 space-y-3.5 shadow-xs">
              ...
              {onInquire && (
                <div className="pt-2 border-t border-line/60">
                  ...
                  >
                    REQUEST RFQ WITH PRODUCT CONTEXT
                  </DigiButton>
                </div>
              )}
            </div>
          ...
          {!loading && !activeAnswer && (
            <div className="rounded-card-md border border-dashed border-line bg-white/70 p-6 flex flex-col items-center justify-center text-center space-y-2.5 min-h-[220px]">
              ...
              <p className="text-[12px] text-stone max-w-sm leading-relaxed font-medium">
                Select a suggested prompt on the left or transmit a custom specification query to synthesize verified product data.
              </p>
            </div>
          )}
        </div>
